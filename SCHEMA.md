...
| parent_user_id | UUID | FOREIGN KEY → users(id) | Hotel owner who created this user |
| status | ENUM | NOT NULL | 'active', 'suspended', 'deleted' |
| created_at | TIMESTAMP | DEFAULT NOW | Account creation date |
| updated_at | TIMESTAMP | AUTO UPDATE | Last modification date |
| created_by | UUID | FOREIGN KEY → users(id) | Who created this account |
...
| created_by | UUID | FOREIGN KEY → users(id) | Who created this role |
| hotel_owner_id | UUID | FOREIGN KEY → users(id) | If scope='hotel_owner', which owner created it |
...
| role_id | UUID | FOREIGN KEY → roles(id), NOT NULL | Which role |
| permission_id | UUID | FOREIGN KEY → permissions(id), NOT NULL | Which permission |
...
| Column Name | Data Type | Constraints | Description |
|------------|-----------|-------------|-------------|
| id | UUID | PRIMARY KEY | Unique hotel identifier |
| user_id | UUID | FOREIGN KEY → users(id), NOT NULL | Hotel owner (user_type='hotel_owner') |
...
| Column Name | Data Type | Constraints | Description |
|------------|-----------|-------------|-------------|
| id | UUID | PRIMARY KEY | Unique access record |
| user_id | UUID | FOREIGN KEY → users(id), NOT NULL | User being granted access |
| hotel_id | UUID | FOREIGN KEY → hotels(id), NOT NULL | Hotel they can access |
| granted_by | UUID | FOREIGN KEY → users(id), NOT NULL | Hotel owner who granted access |
...
| hotel_id | UUID | FOREIGN KEY → hotels(id), NOT NULL | Which hotel this image belongs to |
...
| uploaded_by | UUID | FOREIGN KEY → users(id) | Who uploaded this image |
...
| hotel_id | UUID | FOREIGN KEY → hotels(id), NOT NULL | Which hotel this room belongs to |
...
| status_updated_by | UUID | FOREIGN KEY → users(id) | Who changed the status |
...
|------------|-----------|-------------|-------------|
| id | UUID | PRIMARY KEY | Unique image identifier |
| room_id | UUID | FOREIGN KEY → rooms(id), NOT NULL | Which room this image belongs to |
| image_url | VARCHAR(500) | NOT NULL | Image file path/URL |
...
| created_by | UUID | FOREIGN KEY → users(id) | Who created this guest record |
| hotel_owner_id | UUID | FOREIGN KEY → users(id) | Which hotel owner's system this guest belongs to |
| created_at | TIMESTAMP | DEFAULT NOW | Record creation date |
| updated_at | TIMESTAMP | AUTO UPDATE | Last update date |
...
| room_id | UUID | FOREIGN KEY → rooms(id), NOT NULL | Reserved room |
| hotel_id | UUID | FOREIGN KEY → hotels(id), NOT NULL | Hotel of the room |
| guest_id | UUID | FOREIGN KEY → guests(id), NOT NULL | Guest information |
...
| created_by | UUID | FOREIGN KEY → users(id), NOT NULL | Who created this reservation (staff/owner/admin) |
| cancelled_by | UUID | FOREIGN KEY → users(id) | Who cancelled (if applicable) |
...
| reservation_id | UUID | FOREIGN KEY → reservations(id), NOT NULL | Link to main reservation |
| admin_id | UUID | FOREIGN KEY → users(id), NOT NULL | Super admin who made override |
...
| room_id | UUID | FOREIGN KEY → rooms(id), NOT NULL | Which room |
...
| updated_at | TIMESTAMP | AUTO UPDATE | Last update time |
| updated_by | UUID | FOREIGN KEY → users(id) | Admin who updated |
...
| original_history_id | UUID | NULL | Original history record ID |
| reservation_id | UUID | NULL | Reservation that was affected |
| room_id | UUID | NULL | Room that was reserved |
| hotel_id | UUID | NULL | Hotel of the room |
| admin_id | UUID | NULL | Admin who made override |
