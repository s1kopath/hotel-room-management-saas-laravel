<div class="modal-header">
    <h5 class="modal-title text-brand">🔵 Edit Admin Override Reservation</h5>
    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
</div>
<form method="POST" action="{{ route('reservations.admin-override.update', $reservation->id) }}" id="adminOverrideEditForm">
    @csrf
    @method('PUT')
    <div class="modal-body">
        <div class="alert alert-warning">
            <strong>⚠️ Admin Override:</strong> This reservation overrides normal bookings. Hotel staff cannot modify this reservation.
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control border rounded-3 @error('admin_notes') is-invalid @enderror" 
                ...
                placeholder="Reason for this modification" required>{{ old('admin_notes') }}</textarea>
            <small class="text-muted">This note will be recorded in admin reservation history.</small>
            @if($latestHistory)
            <small class="text-muted d-block mt-1">
                Last note: "{{ $latestHistory->notes }}" ({{ $latestHistory->action_at->format('d/m/Y H:i') }})
            </small>
            @endif
            @error('admin_notes')
                <div class="invalid-feedback">{{ $message }}</div>
    ...
    <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-primary">Update Admin Override</button>
    </div>
</form>
